...
export function AboutPage({ language }: { language: Language }) {
  ...
  const path = '/about';
  return <div className="site-page inner-page" dir={language === 'he' ? 'rtl' : 'ltr'}><SEOHead title={copy.title} description={copy.description} path={localizedPath(path, language)} language={language} keywords={language === 'he' ? ['נמרוד פישר', 'סוכני דאטה', 'יועץ סוכני דאטה', 'בניית סוכני דאטה'] : ['nimrod fisher', 'data agents', 'data agent consultant', 'build data agents']} schema={contentSchema(copy.title, copy.description, localizedPath(path, language), language)} /><SiteHeader language={language} currentPath={path} /><main><section className="inner-hero shell"><div className="section-kicker"><span>ABOUT / NIMROD FISHER</span></div><h1>{copy.h1}</h1><p>{copy.body}</p><a className="ui-button ui-button-primary ui-button-large" href={CALENDAR_URL} target="_blank" rel="noreferrer">{copy.cta} <ArrowUpLeft size={18} /></a></section><section className="inner-grid shell"><div className="inner-grid-heading"><div className="section-kicker"><span>{language === 'he' ? 'העיקרון' : 'THE PRINCIPLE'}</span></div><h2>{language === 'he' ? 'לא מתחילים בטכנולוגיה.' : 'Do not start with technology.'}</h2></div><div className="benefit-list">{copy.points.map((point) => <div className="benefit-item" key={point}><span className="benefit-number">→</span><p>{point}</p></div>)}</div></section></main><footer className="site-footer shell"><span>© 2026 NIMROD FISHER</span><span>DATA AGENTS</span></footer></div>;
}

export function CaseStudiesPage({ language }: { language: Language }) {
  const copy = content[language].cases;
  const path = '/case-studies';
  return <div className="site-page inner-page" dir={language === 'he' ? 'rtl' : 'ltr'}><SEOHead title={copy.title} description={copy.description} path={localizedPath(path, language)} language={language} keywords={language === 'he' ? ['מקרי בוחן AI', 'מקרי בוחן דאטה', 'הטמעת AI בארגון', 'אוטומציה עסקית'] : ['ai consulting case studies', 'data transformation case studies', 'ai implementation examples', 'business automation case study']} schema={contentSchema(copy.title, copy.description, localizedPath(path, language), language)} /><SiteHeader language={language} currentPath={path} /><main><section className="inner-hero shell"><div className="section-kicker"><span>CASE STUDIES / PRACTICE</span></div><h1>{copy.h1}</h1><p>{copy.intro}</p></section><section className="case-grid shell">{copy.items.map(([title, body], index) => <article key={title}><span>0{index + 1}</span><h2>{title}</h2><p>{body}</p><small>{language === 'he' ? 'מקרה לדוגמה — פרטים מלאים מתווספים באישור הלקוח.' : 'Example engagement — detailed metrics are added with client approval.'}</small></article>)}</section><section className="inner-cta shell"><h2>{copy.cta}</h2><a className="ui-button ui-button-dark ui-button-large" href={CALENDAR_URL} target="_blank" rel="noreferrer">{language === 'he' ? 'לקבוע שיחה' : 'Book an assessment'} <ArrowUpLeft size={18} /></a></section></main><footer className="site-footer shell"><span>© 2026 NIMROD FISHER</span><span>DATA AGENTS</span></footer></div>;
}
